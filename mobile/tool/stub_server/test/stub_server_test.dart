...
void main() {
  ...
  group('device auth', () {
    test('device route without a credential is 401', () async {
      final r = await s.handler(req('GET', '/push/get'));
      expect(r.statusCode, 401);
      expect((await bodyOf(r))['data']['error'], 'unauthorized');
    });
    ...
    test('revoke admin route invalidates the token', () async {
      ...
      expect(
        (await s.handler(req('GET', '/push/get', token: t))).statusCode,
        200,
      );
  ...
  group('push requests', () {
    test('seeded pending request is listed with contract shape', () async {
      ...
      expect(
        it.keys,
        containsAll(['id', 'requestId', 'createdAt', 'expiresAt', 'client']),
      );
      expect(it.containsKey('email'), isFalse);
      expect(
        (it['client'] as Map).keys,
        containsAll([
          'userAgentFamily',
          'osFamily',
          'deviceFamily',
    ...
    test('admin __push creates a request and returns its otp', () async {
      ...
      final items =
          (await bodyOf(
                await s.handler(req('GET', '/push/get', token: t)),
              ))['data']
              as List;
    ...
    test(
      'verify with the right otp succeeds and removes the request',
      () async {
        final t = await pair(s);
        final c = (await bodyOf(
          await s.handler(req('POST', '/__push')),
        ))['data'];
        final r = await s.handler(
          req(
            'POST',
            '/push/${c['requestId']}/verify',
            body: {'otp': c['otp']},
            token: t,
          ),
        );
    ...
    test('deny removes the request', () async {
      ...
      final c = (await bodyOf(await s.handler(req('POST', '/__push'))))['data'];
      final r = await s.handler(
        req('POST', '/push/${c['requestId']}/deny', token: t),
      );
      expect(r.statusCode, 200);
      final again = await s.handler(
        req('POST', '/push/${c['requestId']}/deny', token: t),
  ...
  group('security events', () {
    test('first page is newest-first and cursor walks the whole set', () async {
      final t = await pair(s);
      var r = await bodyOf(
        await s.handler(req('GET', '/security-events?limit=50', token: t)),
      );
    ...
    test('seed covers every type and method', () async {
      final t = await pair(s);
      final r = await bodyOf(
        await s.handler(req('GET', '/security-events?limit=200', token: t)),
      );
      final items = r['data']['items'] as List;
      ...
      expect(
        (withActor['actor'] as Map).keys,
        containsAll(['userAgentFamily', 'remoteAddress']),
      );
      expect(withActor['details'], isA<Map>());
    });

    test(
      'malformed cursor and malformed JSON body stay inside the envelope',
      () async {
        final t = await pair(s);
        final c = await s.handler(
          req('GET', '/security-events?before=!!!', token: t),
        );
        expect(c.statusCode, 400);
        expect((await bodyOf(c))['data']['error'], 'bad_cursor');
        final bad = Request(
          'POST',
          Uri.parse('http://stub/push/request-1/verify'),
          headers: {
    ...
    test('accepts an access token as well as a device token', () async {
      final r = await s.handler(
        req('GET', '/security-events', token: 'access-anything'),
      );
      expect(r.statusCode, 200);
    });
  });
