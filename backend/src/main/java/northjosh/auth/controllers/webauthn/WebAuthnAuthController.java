...
public class WebAuthnAuthController {
    ...
    public WebAuthnAuthController(RelyingParty relyingParty,
    ...
    public ResponseEntity<AuthResponse> verifyAuthentication(@RequestBody String credentialJson) {
        try {
            ...
            PublicKeyCredential<AuthenticatorAssertionResponse, ClientAssertionExtensionOutputs> credential = PublicKeyCredential.parseAssertionResponseJson(
                    credentialJson);


            String clientDataStr = new String(credential.getResponse().getClientDataJSON().getBytes(), StandardCharsets.UTF_8);
    ...
    private User findUserByCredentialId(String credentialId) {
        return userRepo.findAll().stream()
                .filter(user -> user.getCredentials().stream()
                        .anyMatch(cred -> java.util.Base64.getUrlEncoder().withoutPadding()
                                .encodeToString(cred.getCredentialId()).equals(credentialId)))
                .findFirst()
                .orElse(null);
    }
    ...
    private void updateSignatureCount(String credentialId, long newSignatureCount) {
        userRepo.findAll().stream()
                ...
                .filter(cred -> java.util.Base64.getUrlEncoder().withoutPadding()
                        .encodeToString(cred.getCredentialId()).equals(credentialId))
                .findFirst()
                .ifPresent(cred -> {
