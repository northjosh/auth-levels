...
public class PushAuthService {
    ...
    public void verify( Map<String, String> dto){
        ...
        PushAuth attempt = pushAuthRepo.findPushAuthByRequestId(requestId).orElseThrow(
                () -> new NoResultException("Login Attempt Doesn't exist")
        );
        ...
        sseEmitters.get(requestId).ifPresent(
                emitter -> {
                    try {
