...
export const Route = createFileRoute("/push-auth")({
  validateSearch: searchSchema,
  component: PushAuthContent,
});

function PushAuthContent() {
  const { clientId, otp } = Route.useSearch();
  ...
  const {
    ...
    clientId: clientId && otp ? clientId : "",
    onSuccess: (token) => {
  ...
  const handleRetry = () => {
    // Re-mounting the push session means tearing down the socket, so a full
    // reload is the reliable way to start a fresh attempt.
